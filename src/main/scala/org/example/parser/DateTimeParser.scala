...
object DateTimeParser extends Serializable {
  ...
  private val knownDateFormats: List[DateTimeFormatter] = List(
    ...
    DateTimeFormatter.ofPattern("dd-MMM-yyyy HH:mm:ss", Locale.US),  // 01-Aug-2020 13:48:07
    DateTimeFormatter.ofPattern("dd.MMM.yyyy'_'HH:mm:ss", Locale.US),  // 01.Aug.2020_13:48:07
    DateTimeFormatter.ofPattern("dd/MMM/yyyy HH:mm:ss", Locale.US),  // 01/Aug/2020 13:48:07
  ...
  def parse(dateStr: String): Option[DateTimeParts] = {
    ...
    val cleanedStr = dateStr
      .replaceAll("^(Mon|Tue|Wed|Thu|Fri|Sat|Sun),?_", "") // Удаляем день недели
      .replaceAll("_[+-]\\d{4}$", "") // Удаляем таймзону (+0300)
